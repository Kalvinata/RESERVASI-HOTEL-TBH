...
<head>
    ...
    <title>Riwayat Check-out - Front Office</title>
...
<body class="bg-slate-50 flex h-screen overflow-hidden text-slate-800">
        ...
        <div>
            ...
            <nav class="p-4 space-y-2 mt-2">
                <a href="/fo/dashboard" class="flex items-center px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800 border-l-4 border-transparent rounded-r-lg font-medium transition-all">
                    <span class="mr-3 text-lg">📋</span> <span class="text-sm">Data Reservasi</span>
                </a>
                <a href="/fo/inhouse" class="flex items-center px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800 border-l-4 border-transparent rounded-r-lg font-medium transition-all">
                    <span class="mr-3 text-lg">🛏️</span> <span class="text-sm">Tamu Menginap</span>
                </a>
                <a href="/fo/history" class="flex items-center px-4 py-3 bg-emerald-500/10 text-emerald-400 border-l-4 border-emerald-500 rounded-r-lg font-medium transition-all">
                    <span class="mr-3 text-lg">📜</span> <span class="text-sm">Riwayat Check-out</span>
                </a>
                <a href="/fo/rooms" class="flex items-center px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800 border-l-4 border-transparent rounded-r-lg font-medium transition-all">
                    <span class="mr-3 text-lg">🔑</span> <span class="text-sm">Status Kamar</span>
                </a>
            </nav>
        </div>
    ...
    <main class="flex-1 overflow-y-auto bg-slate-50 flex flex-col">
        <header class="bg-white border-b border-slate-200 px-8 py-5 sticky top-0 z-10 shadow-sm">
            <h2 class="text-2xl font-bold text-slate-800">Riwayat Tamu Check-out</h2>
            <p class="text-sm text-slate-500">Arsip data tamu yang telah selesai menginap.</p>
        </header>

        <div class="p-8">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    <form action="/fo/history" method="GET" class="flex items-center gap-3">
                        <label class="text-sm font-medium text-slate-600">Filter Tgl Keluar:</label>
                        ...
                        <button type="submit" class="bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-900 transition">Cari</button>
                        <a href="/fo/history" class="text-sm text-red-500 hover:underline">Reset</a>
                    </form>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white border-b border-slate-200 text-xs uppercase text-slate-500">
                            <th class="p-4 font-semibold">Tamu & Kontak</th>
                            <th class="p-4 font-semibold">Kamar</th>
                            <th class="p-4 font-semibold">Tgl Masuk - Keluar</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        <tr class="hover:bg-slate-50/50">
                            <td class="p-4">
                                ...
                                @if($res->user && $res->user->phone)
                                    <a href="https://wa.me/{{ $res->user->phone }}" target="_blank" class="text-xs text-emerald-600 font-medium hover:underline mt-1 flex items-center w-max">
                                        <span class="mr-1">📞</span> {{ $res->user->phone }}
                                    </a>
                                @endif
                            </td>
                            <td class="p-4 text-slate-600 font-medium">No. {{ $res->room->room_number }}</td>
                            <td class="p-4 text-slate-600 font-medium">
                                {{ \Carbon\Carbon::parse($res->check_in_date)->format('d M') }} - {{ \Carbon\Carbon::parse($res->check_out_date)->format('d M Y') }}
                            </td>
                            <td class="p-4 text-center">
                                <span class="bg-slate-200 text-slate-600 px-2 py-1 rounded text-xs font-bold">SELESAI ✓</span>
                            </td>
                        </tr>
                        @endforeach
                        ...
                        <tr>
                            <td colspan="4" class="p-8 text-center text-slate-500">
                                ...
                                <p class="font-medium text-lg">Tidak ada data check-out untuk tanggal tersebut.</p>
                            </td>
                        </tr>
                        @endif
                    </tbody>
